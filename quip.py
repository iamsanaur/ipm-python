...
class QuipClient(object):
    ...
    def get_access_token(self,
        ...
        """
        return self._fetch_json("oauth/access_token",
                                redirect_uri=redirect_uri,
                                code=code,
    ...
    def update_user(self, user_id, picture_url=None):
        return self._fetch_json("users/update",
                                post_data={
                                    "user_id": user_id,
    ...
    def new_folder(self, title, parent_id=None, color=None, member_ids=[]):
        return self._fetch_json("folders/new",
                                post_data={
                                    "title": title,
                                    "parent_id": parent_id,
                                    "color": color,
    ...
    def update_folder(self, folder_id, color=None, title=None):
        return self._fetch_json("folders/update",
                                post_data={
                                    "folder_id": folder_id,
                                    "color": color,
                                    "title": title,
    ...
    def add_folder_members(self, folder_id, member_ids):
        ...
        return self._fetch_json("folders/add-members",
                                post_data={
                                    "folder_id": folder_id,
                                    "member_ids": ",".join(member_ids),
                                })
    ...
    def remove_folder_members(self, folder_id, member_ids):
        ...
        return self._fetch_json("folders/remove-members",
                                post_data={
                                    "folder_id": folder_id,
                                    "member_ids": ",".join(member_ids),
                                })
    ...
    def get_messages(self, thread_id, max_created_usec=None, count=None):
        ...
        """
        return self._fetch_json("messages/" + thread_id,
                                max_created_usec=max_created_usec,
                                count=count)
    ...
    def get_recent_threads(self, max_updated_usec=None, count=None, **kwargs):
        ...
        return self._fetch_json("threads/recent",
                                max_updated_usec=max_updated_usec,
                                count=count,
                                **kwargs)

    def get_matching_threads(self,
                             query,
                             count=None,
                             only_match_titles=False,
                             **kwargs):
        """Returns the recently updated threads for a given user."""
        return self._fetch_json("threads/search",
                                query=query,
                                count=count,
    ...
    def add_thread_members(self, thread_id, member_ids):
        ...
        return self._fetch_json("threads/add-members",
                                post_data={
                                    "thread_id": thread_id,
                                    "member_ids": ",".join(member_ids),
                                })
    ...
    def delete_thread(self, thread_id):
        ...
        return self._fetch_json("threads/delete",
                                post_data={
                                    "thread_id": thread_id,
                                })
    ...
    def remove_thread_members(self, thread_id, member_ids):
        ...
        return self._fetch_json("threads/remove-members",
                                post_data={
                                    "thread_id": thread_id,
                                    "member_ids": ",".join(member_ids),
                                })
    ...
    def new_chat(self, message, title=None, member_ids=[]):
        ...
        initial message."""
        return self._fetch_json("threads/new-chat",
                                post_data={
                                    "message": message,
                                    "title": title,
                                    "member_ids": ",".join(member_ids),
    ...
    def new_document(self,
        ...
        """
        return self._fetch_json("threads/new-document",
                                post_data={
                                    "content": content,
                                    "format": format,
                                    "title": title,
            ...
            for message in reversed(messages):
                ...
                if "files" in message:
                    ...
                    for blob_info in message["files"]:
                        ...
                        new_blob = self.put_blob(original_id,
                                                 blob,
                                                 name=blob_info["name"])
    ...
    def update_spreadsheet_row(self, thread_id, header, value, updates,
                               **args):
        ...
        if row:
            ...
            for head, val in iteritems(updates):
                ...
                if not index or index >= len(ids) or not ids[index]:
                    continue
                response = self.edit_document(thread_id=thread_id,
                                              content=val,
                                              format="markdown",
            ...
            response = self.add_spreadsheet_row(thread_id,
                                                spreadsheet,
                                                updates,
                                                headers=headers,
                                                **args)
